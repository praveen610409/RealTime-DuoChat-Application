# Real-Time Chat App

A simple two-user chat application built using HTML, CSS, and JavaScript.
Users can chat in the same browser window, with messages displayed on the left or right based on the sender.

## Features ✅

Two chat boxes: User A and User B

Right side: Your own messages (green bubble)

Left side: Other user messages (blue bubble)

Scrollable chat area

Responsive design

Press Enter to send messages

Color-coded messages for clarity

Works without any backend

## Demo

Open index.html in your browser to try it out.
(No server or backend required.)

## Installation 🛠️

### Clone the repository:

git clone <https://github.com/praveen610409/RealTime-DuoChat-Application.git>


### Open the folder and launch index.html in your browser.

## Usage 💬

### Type a message in User A or User B input box.

### Press Enter or click Send.

### Messages appear as follows:

    Right (Green): Your own messages

    Left (Blue): Other user's messages

## Folder Structure 📁
Chat-App/
│
├── index.html        # Main HTML file
├── style.css         # Styles for chat layout and message bubbles
└── script.js         # JavaScript for sending/receiving messages

## Customization 🎨

### Change bubble colors in style.css:

    .userA-message { background-color: green; }
    .userB-message { background-color: blue; }


### Adjust chat box size, font, or browser background in style.css.

## Demo 🖼️

Screenshot of the app in action:

![Image](screenshots/chat-demo.png)


## Author ✒️

### Praveen Singh
    💻 Full-Stack Web Developer | MCA Student
    🌐 GitHub
    | 📧 praveen610409@gmail.com

## License 📝

MIT License © 2025 Praveen Singh